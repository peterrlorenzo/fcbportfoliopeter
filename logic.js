...
function handleSlide(e){
	...
	if(hasLost()){
			// Timeout
			setTimeout(()=>{
				alert("Game Over! You have lost the game. Game will now restart.");
				restartGame();
...
function slideLeft(){
	...
	for(let r=0; r < rows; r++){
		...
		for(let c = 0; c < columns; c++){
			...
			// This line is responsible for the animationof slideLeft()
			if(originalRow[c] !== num && num !== 0){
				// specifies the animation style and duration of the animation.
...
function slideUp(){
	...
	for(let c = 0; c< columns; c++){
		...
		// check which tile have change in this column
		// this will record the current position of the tiles that have change
		let changedIndices = [];
		for(let r = 0; r < rows; r++){
	...
	for(let r=0; r< rows ; r++){
		for(let c = 0 ; c < columns ; c++){
			...
			if(board[r][c] == 2048 && is2048Exist == false){
				...
				alert("Outstanding! You have reached 8192!");
				is8192Exist = true;
			}
		}
	}
...
// This will listen to when we touch as screen and assigns the x coordinates of that
...
// Event listener that will listen the touch end
